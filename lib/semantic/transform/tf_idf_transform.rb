      ...
      class << self
        ...
        def transform(matrix)
          ...
          matrix.columns.each_with_index do |document, column_index|
            ...
            document_word_total = document.rows.inject(0.0) {|word_sum, word_count| word_sum + word_count.to_f }

            document.rows.each_with_index do |term_weight, row_index|
              unless term_weight.to_f == 0.0

                number_of_documents_with_term[row_index] ||= self.get_term_document_occurences(row_index, matrix)

                tf = Frequency::term_frequency(term_weight.to_f, document_word_total)
                idf = Frequency::inverse_document_frequency(number_of_documents, number_of_documents_with_term[row_index])
        ...
        def get_term_document_occurences(row, matrix)
          term_document_occurences=0

          rows,cols = matrix.dimensions

          for n in (0...cols)
            if matrix[row,n]>0 #Term appears in document
