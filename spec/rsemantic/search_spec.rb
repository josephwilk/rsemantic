...
module RSemantic
  describe Search do
    ...
    def mock_builder
      @builder ||= double(VectorSpace::Builder)
    end

    def mock_matrix_transformer
      @matrix_transformer ||= double(MatrixTransformer)
    end
    ...
    describe "relating" do
      ...
      it "should find related documents by comparing cosine" do
        ...
        Compare.should_receive(:cosine).with(GSL::Vector[0,1], GSL::Vector[0,1])
        Compare.should_receive(:cosine).with(GSL::Vector[0,1], GSL::Vector[1,0])
